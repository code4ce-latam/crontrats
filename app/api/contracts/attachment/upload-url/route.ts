...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!hasPermission) {
      return NextResponse.json(
        { error: "No tienes permisos para subir archivos adicionales a este contrato" },
        { status: 403 }
      );
    }
    ...
    // Construir storage_path con timestamp para evitar colisiones
    const timestamp = Date.now();
    const sanitizedFileName = original_name.replace(/[^a-zA-Z0-9._-]/g, '_');
    const storagePath = `contracts/${workspaceId}/${contract_id}/attachments/${timestamp}_${sanitizedFileName}`;
    ...
    if (signedUrlError || !signedUrlData) {
      console.error("[Contracts/Attachment/UploadUrl] Error generando signed URL:", signedUrlError);
      return NextResponse.json(
    ...
    if (!signedUrl) {
      console.error("[Contracts/Attachment/UploadUrl] Respuesta inesperada:", signedUrlData);
      return NextResponse.json(
    ...
    console.error("[Contracts/Attachment/UploadUrl] Error inesperado:", error);
    return NextResponse.json(

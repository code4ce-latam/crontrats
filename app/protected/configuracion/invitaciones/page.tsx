...
async function InvitationsContent({ page, pageSizeParam }: { page: number; pageSizeParam?: string }) {
  ...
  const paginatedData = await getWorkspaceInvitationsPaginated(supabase, page, pageSize);

  // Obtener nombres de los usuarios que invitaron
        ...
        if (isInWorkspace) {
          ...
          try {
            ...
            if (!rpcError && userName) {
              ...
              invitedByNames[userId] = userId.substring(0, 8) + '...';
            }
          } catch (rpcError) {

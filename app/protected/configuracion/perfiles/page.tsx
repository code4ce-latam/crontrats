...
import { getWorkspaceContractProfilesPaginated } from "@/lib/supabase/contract-profiles";
import { ContractProfilesList } from "@/components/contract-profiles-list";
...
async function ProfilesContent({ page, pageSizeParam }: { page: number; pageSizeParam?: string }) {
  ...
  const paginatedData = await getWorkspaceContractProfilesPaginated(supabase, page, pageSize);

  return (
    <ContractProfilesList 
    ...
    <div className="flex-1 w-full flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-muted-foreground">
          Gestiona los perfiles de contratos de tu workspace.
        </p>
      </div>
      ...
      >
        <ProfilesContent page={validPage} pageSizeParam={params.pageSize} />
      </Suspense>
    </div>
  );
}

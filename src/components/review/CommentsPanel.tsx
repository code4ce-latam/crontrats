...
export function CommentsPanel({
  ...
  React.useEffect(() => {
    if (selected && !editingId && !selected.text.trim()) {
      ...
      <div className="border-t border-zinc-200 bg-zinc-50 px-3 py-3">
        <div className="mb-2">
          ...
          <p className="text-[11px] text-zinc-500">
            Haz clic en un comentario arriba para editarlo o ver su ubicación.
          </p>
        </div>
        ...
        <div className="space-y-2">
          <Input
            ...
            value={
              selected
                ? `Página ${selected.page} · x=${Math.round(
                    selected.x * 100
                  )}%, y=${Math.round(selected.y * 100)}%`
                : "Selecciona un comentario para ver su ubicación"
            }
            className="text-xs"
          />
          {selected && (
            <p className="text-[10px] text-zinc-500">
              Haz clic en el comentario arriba para editarlo directamente.
            </p>
          )}
        </div>
      </div>

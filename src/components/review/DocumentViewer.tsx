...
export function DocumentViewer() {
  ...
  useEffect(() => {
    const handler = (ev: KeyboardEvent) => {
      if (ev.key === "Escape") {
        setState((prev) => ({ ...prev, toolMode: "SELECT" }));
      }
  ...
  const handleSave = () => {
    ...
    // Mostrar feedback visual de guardado (opcional: toast o mensaje)
    // Por ahora, el estado se actualiza y se refleja en la UI automáticamente
  };
  ...
  const handleCreateAnchorAt = (page: number, x: number, y: number) => {
    ...
    setState((prev) => ({
      ...prev,
      comments: [...prev.comments, emptyComment],
      selectedId: id,
      draftText: "",
      toolMode: "SELECT",
    }));
      ...
      >
        <div className="flex-1 overflow-auto rounded-xl border border-zinc-200 bg-zinc-50 p-6">
          <DocumentViewerPages
            ...
            toolMode={state.toolMode}
            onCreateAnchor={handleCreateAnchorAt}
